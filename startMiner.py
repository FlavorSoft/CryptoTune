...
class StartMiner:
    ...
    def Start(self, algo, memOCs, coreUCs):
        ...
        if not self.isWindows:
            parameters = self.config["parametersUnix"].replace("#core#", str(coreUCs)).replace("#mem#", str(memOCs)).replace("#fan#", str(self.fans)).replace("#dev#", str(self.devIds)).replace("#worker#", self.workerName).replace("#port#", str(self.config["api"]["port"])).replace("#host#", self.config["api"]["host"]).replace("#ALGO#", algo)
